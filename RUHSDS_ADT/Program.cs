...
namespace RUHSDS_ADT
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            // 14001 = Prod (ADT)
            // 14101 = Test (ADT)
            HL7TCPListener listener = new HL7TCPListener(Convert.ToInt32(System.Configuration.ConfigurationManager.AppSettings["ListenerPort"].ToString()));
            listener.SendACK = true; //sendACK;

            if (filePath != null)
            {
                listener.FilePath = "C:\\HL7";
            }

            if (!listener.Start())
            {
                Console.WriteLine("Listener did not start");
                Console.ReadLine();
            }
